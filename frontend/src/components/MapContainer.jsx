...
const getGlowStyle = (type) => {
  ...
  if (category === 'music') return 'bg-amber-500/70 shadow-[0_0_60px_30px_rgba(245,158,11,0.5)]';
  if (category === 'food') return 'bg-orange-500/70 shadow-[0_0_60px_30px_rgba(249,115,22,0.5)]';
  if (category === 'gaming') return 'bg-cyan-500/70 shadow-[0_0_60px_30px_rgba(6,182,212,0.5)]';
  if (category === 'fitness') return 'bg-emerald-500/70 shadow-[0_0_60px_30px_rgba(16,185,129,0.5)]';
...
const MapContainer = () => {
  ...
  useEffect(() => {
    ...
    buzzes.forEach(buzz => {
      ...
      if (buzz.distance > 3000) {
        ...
        el.className = 'h-10 rounded-full flex items-center bg-zinc-900/60 border border-zinc-700/50 shadow-xl backdrop-blur-md px-3 cursor-pointer hover:bg-zinc-800 transition-all';
        el.innerHTML = `<span class="text-lg mr-2">${buzz.icon || "📍"}</span><div class="flex flex-col"><span class="text-[8px] font-black text-zinc-500 uppercase tracking-widest">${buzz.type}</span><span class="text-[9px] font-bold text-zinc-400 truncate max-w-[100px]">${buzz.zone || "Unknown"}</span></div>`;
        el.onclick = () => setSelectedBuzz(buzz);
      } else if (buzz.distance > 200) {
        el.className = 'h-12 rounded-full flex items-center bg-zinc-900 border border-zinc-600 shadow-2xl backdrop-blur-md overflow-hidden transition-all duration-500 max-w-[48px] cursor-pointer hover:border-white/50';
        el.innerHTML = `
          ...
          <div class="text-content flex flex-col justify-center opacity-0 whitespace-nowrap pl-1 pr-5">
            <span class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">${buzz.zone}</span>
        ...
        el.onclick = () => {
          if (!expanded) {
            ...
            el.querySelector('.text-content').classList.replace('opacity-0', 'opacity-100');
            expanded = true;
          } else { setSelectedBuzz(buzz); }
        };
      } else {
        ...
        el.innerHTML = `
          <div class="w-8 h-8 rounded-full bg-black/10 flex items-center justify-center mr-3"><span>${buzz.icon || "📍"}</span></div>
          <span>${buzz.isSecret ? 'SECRET EVENT' : buzz.title}</span>`;
        el.onclick = () => { setSelectedBuzz(buzz); setIsUnlocked(false); setPasswordAttempt(""); setPasswordError(false); };
      }
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden bg-zinc-950 text-white font-sans antialiased">
      ...
      {/* 🗺️ MAP LAYER */}
      <div className={`absolute inset-0 transition-all duration-1000 ${locationStatus !== 'ready' || isScanning ? 'blur-2xl opacity-30 scale-110' : 'blur-0 opacity-100'}`}>
      ...
      {isScanning && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center pointer-events-none">
          <div className="absolute w-[600px] h-[600px] border border-green-500/20 rounded-full animate-[ping_4s_linear_infinite]" />
          <div className="absolute w-[400px] h-[400px] border border-green-500/40 rounded-full animate-[ping_4s_linear_infinite]" style={{ animationDelay: '1s' }} />
          
          <div className="relative z-10 flex flex-col items-center bg-black/60 backdrop-blur-xl px-10 py-8 rounded-[3rem] border border-green-500/30 shadow-[0_0_50px_rgba(34,197,94,0.2)]">
            <span className="text-5xl mb-4 animate-spin" style={{ animationDuration: '3s' }}>📡</span>
            <h2 className="text-green-500 font-mono text-xs font-black uppercase tracking-[0.3em] mb-2">Intercepting Signals...</h2>
            <p className="text-white font-black text-4xl italic tracking-tighter">
              {foundCount} <span className="text-green-500/50">FOUND</span>
            </p>
          </div>
        </div>
      )}
      ...
      {locationStatus !== 'ready' && !isScanning && (
        <div className="absolute inset-0 z-[70] flex flex-col items-center justify-center pointer-events-none bg-black/40 backdrop-blur-sm">
          ...
          <div className="relative text-7xl font-black italic uppercase tracking-tighter select-none mb-16">
            ...
            <h1 className="absolute left-0 top-0 text-white/10 w-full text-center">Rumour</h1>
            <h1 
          ...
          <div className="h-24 flex flex-col items-center justify-center">
            {locationStatus === 'locating' ? (
              <div className="flex flex-col items-center animate-fadeIn">
                 <div className="w-12 h-12 border-4 border-zinc-800 border-t-white rounded-full animate-spin mb-4" />
                 <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 animate-pulse">Syncing local grid...</p>
              </div>
            ) : locationStatus === 'success' ? (
              <div className="flex flex-col items-center animate-bounceScale">
                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-4 shadow-2xl">
                  <span className="text-black text-2xl font-black">✓</span>
                </div>
                <p className="text-[10px] font-black uppercase tracking-widest text-white">Grid Locked</p>
              </div>
            ) : null}
          </div>
    ...
    {/* HEADER UI */}
      <header className={`absolute top-0 left-0 right-0 p-6 flex justify-between items-start z-40 transition-opacity duration-700 ${locationStatus === 'ready' ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex gap-2">
          <button 
            ...
            disabled={isScanning}
            className="bg-green-500 text-black px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest shadow-[0_0_20px_rgba(34,197,94,0.4)] active:scale-95 transition-all disabled:opacity-50"
          >
            {isScanning ? "Scanning..." : "Initiate Scan"}
          </button>

          <button onClick={() => auth.signOut()} className="bg-zinc-900 border border-zinc-800 px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-2xl">Leave</button>
        </div>
      </header>
      ...
      {showIntelReport && buzzes.length > 0 && (
        <div className="absolute inset-0 z-[100] flex items-end justify-center pb-8 px-4 pointer-events-none">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md pointer-events-auto" onClick={() => setShowIntelReport(false)} />
          
          <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-2xl pointer-events-auto animate-slideUp">
            <div className="bg-green-500 py-2 text-center">
              <p className="text-[9px] font-black text-black uppercase tracking-[0.3em]">Signal Intel Report Locked</p>
            </div>

            <div className="p-8">
              <div className="mb-8 p-6 bg-white rounded-3xl text-black shadow-xl">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">Closest Connection</p>
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-2xl font-black uppercase tracking-tighter leading-none mb-1">
                      {buzzes[0].distance < 200 ? buzzes[0].title : `${buzzes[0].type} In ${buzzes[0].zone}`}
                    </h3>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase">{buzzes[0].distance < 1000 ? `${Math.round(buzzes[0].distance)}m` : `${(buzzes[0].distance / 1000).toFixed(1)}km`} Away</p>
                  </div>
              ...
              <div className="space-y-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500 px-2 mb-4">Other Nearby Signatures</p>
                {buzzes.slice(1, 5).map((buzz) => (
                  <div key={buzz.id} className="flex items-center justify-between bg-zinc-950/50 border border-zinc-800/50 p-4 rounded-2xl">
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{buzz.icon}</span>
                      <div className="flex flex-col">
                        <span className="text-xs font-black text-white uppercase tracking-tight">
                          {buzz.distance < 200 ? buzz.title : `${buzz.type} Reveal`}
                        </span>
                        <span className="text-[9px] text-zinc-600 font-bold uppercase">{buzz.zone || "Zone Redacted"}</span>
                      </div>
                    </div>
                    <span className="text-[10px] font-mono text-zinc-400">
                      {buzz.distance < 1000 ? `${Math.round(buzz.distance)}m` : `${(buzz.distance / 1000).toFixed(1)}km`}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <button 
                ...
                className="w-full mt-8 bg-zinc-800 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-zinc-700 transition-all"
              >
                Dismiss Report
              </button>
            </div>
      ...
      {selectedBuzz && (
        <div className="absolute inset-0 z-[110] flex items-end justify-center pb-6 px-4 pointer-events-none">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-xl pointer-events-auto" onClick={closeEverything} />
            ...
            {isCheckedIn ? (
              ...
              <>
                {selectedBuzz.distance > 1000 ? (
                   <div className="relative min-h-[400px] flex flex-col items-center justify-center p-8 text-center bg-zinc-950/95">
                     <span className="text-5xl mb-6 opacity-50 animate-pulse">📡</span>
                     <h3 className="text-zinc-400 font-black text-2xl uppercase tracking-widest mb-2">Faint Signal</h3>
                     <p className="text-zinc-400 text-xs px-4">Signals in <span className="text-white font-black">{selectedBuzz.zone}</span>. Move closer to intercept data.</p>
                     <button onClick={closeEverything} className="mt-10 bg-zinc-800 text-white px-10 py-3 rounded-full font-black text-[10px] uppercase tracking-widest">Keep Walking</button>
                   </div>
                ) : selectedBuzz.distance > 200 ? (
                   <div className="relative min-h-[400px] flex flex-col items-center justify-center p-8 text-center bg-zinc-950/95 border-b-[6px] border-green-500">
                      <span className="text-5xl mb-6">🪝</span>
                      <h3 className="text-white font-black text-3xl uppercase tracking-widest mb-6">Teaser Intercepted</h3>
                      <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6 w-full mb-6">
                         <p className="text-white font-mono text-sm leading-relaxed italic">"{selectedBuzz.teaser}"</p>
                      </div>
                      <button onClick={closeEverything} className="mt-4 bg-white text-black px-10 py-3 rounded-full font-black text-[10px] uppercase">Understood</button>
                   </div>
                ) : (
                   <>
                     {selectedBuzz.isSecret && !isUnlocked ? (
                       <div className="relative flex flex-col items-center justify-center p-10 text-center min-h-[450px] bg-zinc-950 border-b-[6px] border-red-500 overflow-hidden">
                          <div className="absolute inset-0 opacity-30 blur-xl scale-125 bg-cover bg-center" style={{ backgroundImage: `url(${selectedBuzz.image})` }} />
                          <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/40 to-transparent" />
                          <div className="relative z-10 w-full flex flex-col items-center">
                            <span className="text-6xl mb-6 animate-pulse">🤫</span>
                            <h3 className="text-red-500 font-black text-3xl uppercase tracking-widest mb-8">Secret Door</h3>
                            <form onSubmit={(e) => {
                              e.preventDefault();
                              if (passwordAttempt === selectedBuzz.password) { setIsUnlocked(true); setPasswordError(false); }
                              else { setPasswordError(true); setPasswordAttempt(""); }
                            }} className="w-full flex flex-col gap-4">
                              {passwordError && (
                                <div className="bg-red-500/10 border border-red-500/30 py-2 rounded-xl animate-pulse">
                                  <p className="text-red-500 font-mono text-[10px] uppercase font-black">⚠️ Access Denied</p>
                                </div>
                              )}
                              <input 
                                type="text" value={passwordAttempt} 
                                onChange={e => { setPasswordAttempt(e.target.value); setPasswordError(false); }} 
                                placeholder="Password..." 
                                className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl py-5 text-white text-center font-mono outline-none focus:border-red-500 transition-all text-sm tracking-widest" 
                              />
                              <button type="submit" className="bg-red-500 text-white font-black py-5 rounded-2xl uppercase tracking-widest">Knock</button>
                            </form>
                            <button onClick={closeEverything} className="mt-6 text-zinc-600 text-[10px] font-black uppercase tracking-widest hover:text-white transition-colors">Walk away</button>
                          </div>
                       </div>
                     ) : (
                       <div>
      ...
      {/* 📖 FIELD PROTOCOL TAB */}
      <div className={`absolute bottom-8 left-4 z-40 transition-all duration-500 ${isLegendOpen ? 'opacity-0 pointer-events-none -translate-x-14' : 'opacity-100 translate-x-0'}`}>
        <button 
          onClick={() => setIsLegendOpen(true)}
          className="group flex items-center gap-3 bg-zinc-900/90 border border-zinc-800 p-3 pr-6 rounded-full backdrop-blur-xl shadow-2xl hover:border-zinc-500 transition-all active:scale-95"
        >
          <div className="w-10 h-10 bg-zinc-800 rounded-full flex items-center justify-center text-xl group-hover:bg-zinc-700 transition-colors">
            📖
          </div>
          <div className="flex flex-col items-start">
            <span className="text-[10px] font-black text-white uppercase tracking-widest">Field Protocol</span>
            <span className="text-[8px] font-mono text-zinc-500 uppercase tracking-widest">v3.0.48</span>
          </div>
        </button>
      </div>
